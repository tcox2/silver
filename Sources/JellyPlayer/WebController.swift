...
final class WebController: @unchecked Sendable {
    ...
    const pageGeneration='__SILVER_SERVER_GENERATION__';let pageInvalidated=false,generationFailures=0,statusInterval,catalogInterval,generationInterval;
    ...
    async function checkPageGeneration(){if(pageInvalidated)return;try{let r=await fetch('/api/page-generation',{cache:'no-store'});if(!r.ok)throw Error('server unavailable');let value=await r.json();if(value.generation!==pageGeneration){invalidatePage();return}generationFailures=0}catch(e){if(++generationFailures>=2)invalidatePage()}}
