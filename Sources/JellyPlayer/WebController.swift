...
final class WebController: @unchecked Sendable {
        ...
        do {
            ...
            case ("GET", "/"):
                send(
                    ...
                    body: Self.page.replacingOccurrences(
                        of: "__SILVER_SERVER_GENERATION__",
                        with: generation
                    ),
                    to: connection
                )
            case ("GET", "/api/page-generation"):
                sendJSON(["generation": generation], to: connection)
            ...
            case ("POST", "/api/control/amplifier/volume-down"):
                ...
                guard let model else { throw CinemaError.loxoneUnavailable }
                sendJSON(try await model.adjustAmplifierVolume(by: -2), to: connection)
            case ("POST", "/api/control/reboot"):
                let command = try JSONDecoder().decode(RebootCommand.self, from: request.body)
                guard command.confirm else { throw HostControlError.rebootNotConfirmed }
                SilverLog.info("Web command host reboot")
                try requestHostReboot()
                sendJSON(["accepted": true], to: connection)
            case ("POST", "/api/item"):
    ...
    private func requestHostReboot() throws {
        let process = Process()
    ...
    async function rebootHost(){if(!confirm('Reboot the machine running Silver?'))return;rebootButton.disabled=true;rebootResult.textContent='Requesting reboot…';error.textContent='';try{await command('/api/control/reboot',{confirm:true});invalidatePage()}catch(e){error.textContent=e.message;rebootResult.textContent='Reboot failed';rebootButton.disabled=false}}
    ...
    refreshButton.onclick=()=>location.reload();rebootButton.onclick=rebootHost;volumeDownButton.onclick=()=>adjustAmplifierVolume(-2);volumeUpButton.onclick=()=>adjustAmplifierVolume(2);
    statusTabButton.onclick=()=>selectTab('status');libraryTabButton.onclick=()=>selectTab('library');controlTabButton.onclick=()=>selectTab('control');youtubeTabButton.onclick=()=>selectTab('youtube');projectorOnButton.onclick=()=>setProjectorPower(true);projectorOffButton.onclick=()=>setProjectorPower(false);backButton.onclick=()=>{details.hidden=true;libraryList.hidden=false;selectedItem=null};detailPlayButton.onclick=playSelected;pauseButton.onclick=()=>playbackCommand('/api/pause');resumeButton.onclick=()=>playbackCommand('/api/resume');stopButton.onclick=()=>playbackCommand('/api/stop');firstPageButton.onclick=()=>loadLibraryPage(0);previousPageButton.onclick=()=>loadLibraryPage(libraryPage-1);nextPageButton.onclick=()=>loadLibraryPage(libraryPage+1);lastPageButton.onclick=()=>loadLibraryPage(libraryPageCount-1);search.oninput=()=>{clearTimeout(searchTimer);searchTimer=setTimeout(()=>loadLibraryPage(0,true),250)};seek.oninput=()=>{if(dragging)elapsed.textContent=clock(seek.value)};statusInterval=setInterval(status,1000);catalogInterval=setInterval(catalogStatus,1000);generationInterval=setInterval(checkPageGeneration,1000);status();catalogStatus();checkPageGeneration();
    </script>
    """#
}
...
private struct RebootCommand: Decodable { let confirm: Bool }

private enum HostControlError: LocalizedError {
    case rebootNotConfirmed

    var errorDescription: String? {
        switch self {
        case .rebootNotConfirmed: "Host reboot was not confirmed."
        }
    }
}
