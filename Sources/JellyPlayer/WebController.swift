...
final class WebController: @unchecked Sendable {
    ...
    async function catalogStatus(){if(catalogStatusBusy)return;catalogStatusBusy=true;try{let r=await fetch('/api/catalog-status'),s=await r.json();if(!r.ok)throw Error(s.error||'Unable to connect to Jellyfin');if(s.error)error.textContent=s.error;if(s.configured&&s.revision!==libraryRevision)await loadLibraryPage(true)}catch(e){error.textContent=e.message}finally{catalogStatusBusy=false}}
    ...
    statusTabButton.onclick=()=>selectTab('status');libraryTabButton.onclick=()=>selectTab('library');backButton.onclick=()=>{details.hidden=true;libraryList.hidden=false;selectedItem=null};detailPlayButton.onclick=playSelected;pauseButton.onclick=()=>playbackCommand('/api/pause');resumeButton.onclick=()=>playbackCommand('/api/resume');stopButton.onclick=()=>playbackCommand('/api/stop');loadMoreButton.onclick=()=>loadLibraryPage(false);search.oninput=()=>{clearTimeout(searchTimer);searchTimer=setTimeout(()=>loadLibraryPage(true),250)};seek.oninput=()=>{if(dragging)elapsed.textContent=clock(seek.value)};setInterval(status,1000);setInterval(catalogStatus,1000);status();catalogStatus();
    </script>
    """#
}
