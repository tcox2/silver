...
final class WebController: @unchecked Sendable {
    ...
    async function refreshLibrary(revision){if(!hasCachedCatalog)error.textContent='Loading library…';try{let r=await fetch('/api/library'),result=await r.json();if(!r.ok)throw Error(result.error||'Catalog refresh failed');if(!result.configured)return;renderLibrary(result);saveLibrary(result);libraryRevision=revision}catch(e){error.textContent=hasCachedCatalog?`Catalog refresh failed · showing saved catalog\n${e.message}`:e.message}};
    function renderCatalogStatus(s){catalogMessage.textContent=s.progress||'Loading Jellyfin catalogue…';reloadLibraryButton.disabled=!s.configured||s.refreshing;if(s.totalItems>0){catalogProgress.max=s.totalItems;catalogProgress.value=Math.min(s.loadedItems,s.totalItems);catalogCount.textContent=`${s.loadedItems} of ${s.totalItems} items`}else{catalogProgress.removeAttribute('value');catalogCount.textContent=s.loadedItems?`${s.loadedItems} items loaded`:''}if(s.error)error.textContent=s.error}
    async function catalogStatus(){if(catalogStatusBusy)return;catalogStatusBusy=true;try{let r=await fetch('/api/catalog-status'),s=await r.json();if(!r.ok)throw Error(s.error||'Unable to read catalogue status');renderCatalogStatus(s);if(s.configured&&s.revision!==libraryRevision)await refreshLibrary(s.revision)}catch(e){error.textContent=e.message}finally{catalogStatusBusy=false}}
