...
final class WebController: @unchecked Sendable {
    ...
    <style>body{margin:0;background:#080808;color:#eee;font:16px system-ui;max-width:900px;padding:32px;margin:auto}h1,h2{font-weight:300}input,button,select{font:inherit;padding:12px;border-radius:8px;border:1px solid #444;background:#181818;color:white}button{cursor:pointer}button:disabled{cursor:not-allowed;opacity:.4}.tabs{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:20px 0}.tabs button.active{background:#eee;color:#111;border-color:#eee}.item{display:flex;justify-content:space-between;align-items:center;gap:14px;padding:14px 0;border-bottom:1px solid #292929}.item.incompatible{color:#999}.muted{color:#999;font-size:13px}.reason{color:#e6a85c;font-size:12px;margin-top:3px}#error{color:#ff7777;white-space:pre-wrap}#output,#now,#details,#catalogStatus{background:#141414;border:1px solid #303030;border-radius:12px;padding:20px;margin:22px 0}#catalogStatus{padding:18px 20px}.catalogHeader{display:flex;align-items:center;justify-content:space-between;gap:16px}.catalogHeader h2{font-size:20px;font-weight:500;margin:4px 0 0}.catalogHeader button{flex:none}#catalogProgress{box-sizing:border-box;width:100%;height:10px;margin:14px 0 5px;accent-color:#eee}#catalogProgress[hidden]{display:none}#catalogCount{margin-top:7px}#search{box-sizing:border-box;width:100%;margin:14px 0 4px}#librarySummary{margin:10px 0}#loadMoreButton{display:block;margin:18px auto}.facts{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}.fact{background:#252525;border-radius:999px;padding:5px 10px;font-size:13px}.timeline{display:grid;grid-template-columns:60px 1fr 60px;gap:10px;align-items:center;font-variant-numeric:tabular-nums}.timeline input{padding:0;width:100%;accent-color:#eee}.detail-grid{display:grid;grid-template-columns:max-content 1fr;gap:8px 18px;margin:20px 0}.detail-grid dt{color:#999}.detail-grid dd{margin:0;overflow-wrap:anywhere}#subtitleSelector{box-sizing:border-box;width:100%;margin:8px 0 16px}.overview{line-height:1.5}@media(max-width:600px){body{padding:18px}.catalogHeader{align-items:flex-start}.catalogHeader h2{font-size:18px}}</style>
    <h1>home cinema</h1><p id="error"></p><section id="app"><nav class="tabs"><button id="statusTabButton" class="active">Status</button><button id="libraryTabButton">Library</button></nav><div id="statusTab"><article id="output"><div class="muted">CURRENT OUTPUT</div><h2 id="outputResolution">Unavailable</h2><div id="outputFacts" class="facts"></div></article><article id="now" hidden><div class="muted">NOW PLAYING</div><h2 id="nowTitle"></h2><div id="nowDetail" class="muted"></div><div id="facts" class="facts"></div><div class="timeline"><span id="elapsed">0:00</span><input id="seek" type="range" min="0" value="0" step="0.1"><span id="duration">0:00</span></div><p><button id="pauseButton">Pause</button> <button id="resumeButton">Resume</button> <button id="stopButton">Stop playback</button></p></article><div id="idle" class="muted">Nothing playing</div></div><div id="libraryTab" hidden><div id="libraryList"><article id="catalogStatus"><div class="catalogHeader"><div><div class="muted">JELLYFIN CATALOGUE</div><h2 id="catalogMessage">Starting…</h2></div><button id="reloadLibraryButton">Reload now</button></div><progress id="catalogProgress"></progress><div id="catalogCount" class="muted"></div></article><input id="search" type="search" placeholder="Search Jellyfin library" autocomplete="off" aria-label="Search Jellyfin library"><div id="librarySummary" class="muted"></div><div id="items"><p class="muted">Loading Jellyfin catalogue…</p></div><button id="loadMoreButton" hidden>Load more</button></div><article id="details" hidden><button id="backButton">← Library</button><h2 id="detailTitle"></h2><div id="detailSubtitle" class="muted"></div><div id="detailFacts" class="facts"></div><p id="detailOverview" class="overview"></p><dl id="fileDetails" class="detail-grid"></dl><label for="subtitleSelector">Subtitles</label><select id="subtitleSelector"></select><p><button id="detailPlayButton">Play</button></p><p id="detailReason" class="reason"></p></article></div></section>
    <script>
    const $=id=>document.getElementById(id),error=$('error'),statusTab=$('statusTab'),libraryTab=$('libraryTab'),statusTabButton=$('statusTabButton'),libraryTabButton=$('libraryTabButton'),outputResolution=$('outputResolution'),outputFacts=$('outputFacts'),now=$('now'),idle=$('idle'),nowTitle=$('nowTitle'),nowDetail=$('nowDetail'),facts=$('facts'),seek=$('seek'),elapsed=$('elapsed'),duration=$('duration'),pauseButton=$('pauseButton'),resumeButton=$('resumeButton'),stopButton=$('stopButton'),search=$('search'),itemsElement=$('items'),librarySummary=$('librarySummary'),loadMoreButton=$('loadMoreButton'),libraryList=$('libraryList'),details=$('details'),backButton=$('backButton'),detailTitle=$('detailTitle'),detailSubtitle=$('detailSubtitle'),detailFacts=$('detailFacts'),detailOverview=$('detailOverview'),fileDetails=$('fileDetails'),subtitleSelector=$('subtitleSelector'),detailPlayButton=$('detailPlayButton'),detailReason=$('detailReason'),catalogMessage=$('catalogMessage'),catalogProgress=$('catalogProgress'),catalogCount=$('catalogCount'),reloadLibraryButton=$('reloadLibraryButton');
    ...
    let libraryItems=[],selectedItem=null,libraryRevision=-1,libraryTotal=0,libraryPageBusy=false,libraryRequestGeneration=0,catalogStatusBusy=false,searchTimer=null;
    ...
    async function loadLibraryPage(reset=false){if(!reset&&libraryPageBusy)return;let generation=reset?++libraryRequestGeneration:libraryRequestGeneration,query=search.value.trim();libraryPageBusy=true;loadMoreButton.disabled=true;if(reset){libraryItems=[];libraryTotal=0;librarySummary.textContent='';itemsElement.innerHTML='<p class="muted">Loading library…</p>'}try{let result=await command('/api/library-page',{query,offset:reset?0:libraryItems.length,limit:libraryPageSize});if(generation!==libraryRequestGeneration||query!==search.value.trim())return;if(reset)libraryItems=[];libraryItems.push(...(result.items||[]));libraryTotal=result.total;libraryRevision=result.revision;error.textContent='';renderItems()}catch(e){if(generation!==libraryRequestGeneration)return;error.textContent=e.message;if(reset)itemsElement.innerHTML='<p class="muted">Unable to load library.</p>'}finally{if(generation===libraryRequestGeneration){libraryPageBusy=false;loadMoreButton.disabled=false}}}
    function renderCatalogStatus(s){let browserLoading=s.configured&&s.revision!==libraryRevision,catalogLoading=!s.configured;catalogMessage.textContent=browserLoading?'Loading library page…':(s.progress||'Connecting to Jellyfin…');reloadLibraryButton.disabled=!s.configured||browserLoading;catalogProgress.hidden=!catalogLoading&&!browserLoading;catalogProgress.removeAttribute('value');catalogCount.textContent=s.configured&&!browserLoading?'Pages load directly from Jellyfin':'';if(s.error)error.textContent=s.error}
    async function catalogStatus(){if(catalogStatusBusy)return;catalogStatusBusy=true;try{let r=await fetch('/api/catalog-status'),s=await r.json();if(!r.ok)throw Error(s.error||'Unable to read catalogue status');renderCatalogStatus(s);if(s.configured&&s.revision!==libraryRevision){await loadLibraryPage(true);renderCatalogStatus(s)}}catch(e){error.textContent=e.message}finally{catalogStatusBusy=false}}
    async function reloadLibrary(){reloadLibraryButton.disabled=true;error.textContent='';await loadLibraryPage(true);reloadLibraryButton.disabled=false}
    ...
    statusTabButton.onclick=()=>selectTab('status');libraryTabButton.onclick=()=>selectTab('library');backButton.onclick=()=>{details.hidden=true;libraryList.hidden=false;selectedItem=null};detailPlayButton.onclick=playSelected;pauseButton.onclick=()=>playbackCommand('/api/pause');resumeButton.onclick=()=>playbackCommand('/api/resume');stopButton.onclick=()=>playbackCommand('/api/stop');reloadLibraryButton.onclick=reloadLibrary;loadMoreButton.onclick=()=>loadLibraryPage(false);search.oninput=()=>{clearTimeout(searchTimer);searchTimer=setTimeout(()=>loadLibraryPage(true),250)};seek.oninput=()=>{if(dragging)elapsed.textContent=clock(seek.value)};setInterval(status,1000);setInterval(catalogStatus,1000);status();catalogStatus();
    </script>
    """#
}
