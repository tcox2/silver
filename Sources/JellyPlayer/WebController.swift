...
final class WebController: @unchecked Sendable {
    ...
    const youtubeTabButton=document.createElement('button'),youtubeTab=document.createElement('div'),youtubeItems=document.createElement('div');youtubeTabButton.id='youtubeTabButton';youtubeTabButton.textContent='YouTube';document.querySelector('.tabs').append(youtubeTabButton);document.querySelector('.tabs').style.gridTemplateColumns='repeat(3,1fr)';youtubeTab.id='youtubeTab';youtubeTab.hidden=true;youtubeTab.innerHTML='<h2>Downloaded videos</h2><p class="muted">Videos downloaded by Zorg. Select Play to show one on the home cinema.</p>';youtubeTab.append(youtubeItems);document.querySelector('#app').append(youtubeTab);
    let youtubeLoaded=false,youtubeBusy=false;
    selectTab=function(name){let showStatus=name==='status',showLibrary=name==='library',showYouTube=name==='youtube';statusTab.hidden=!showStatus;libraryTab.hidden=!showLibrary;youtubeTab.hidden=!showYouTube;statusTabButton.classList.toggle('active',showStatus);libraryTabButton.classList.toggle('active',showLibrary);youtubeTabButton.classList.toggle('active',showYouTube);if(showYouTube)loadYouTube()}
    async function loadYouTube(){if(youtubeBusy)return;youtubeBusy=true;youtubeItems.innerHTML='<p class="muted">Loading downloaded videos…</p>';try{let r=await fetch('/api/youtube'),videos=await r.json();if(!r.ok)throw Error(videos.error||'Unable to load YouTube videos');youtubeLoaded=true;youtubeItems.innerHTML=videos.length?videos.map(v=>`<div class="item"><div>${html(v.title)}<div class="muted">${html([v.width&&v.height?`${v.width}×${v.height}`:null,v.frameRate?`${v.frameRate.toFixed(3)} fps`:null,v.dynamicRange,v.videoCodec,v.audioCodec,bytes(v.sizeBytes)].filter(Boolean).join(' · '))}</div></div><button data-youtube-id="${html(v.videoId)}">Play</button></div>`).join(''):'<p class="muted">No downloaded videos.</p>';document.querySelectorAll('[data-youtube-id]').forEach(b=>b.onclick=()=>playYouTube(b.dataset.youtubeId))}catch(e){error.textContent=e.message;youtubeItems.innerHTML='<p class="muted">Unable to load downloaded videos.</p>'}finally{youtubeBusy=false}}
