...
final class WebController: @unchecked Sendable {
    ...
    private func route(_ data: Data, connection: NWConnection) async {
        ...
        do {
            ...
            case ("POST", "/api/reload-library"):
                guard let model else { throw CinemaError.incompatible }
                try await model.requestCatalogRefresh()
    ...
    let libraryItems=[],selectedItem=null,hasCachedCatalog=false,libraryRevision=-1,catalogStatusBusy=false;
    ...
    async function refreshLibrary(revision){if(!hasCachedCatalog){error.textContent='';itemsElement.innerHTML='<p class="muted">Loading catalogue into browser…</p>'}try{let r=await fetch('/api/library'),result=await r.json();if(!r.ok)throw Error(result.error||'Catalogue download failed');if(!result.configured)return;renderLibrary(result);saveLibrary(result);libraryRevision=revision}catch(e){error.textContent=hasCachedCatalog?`Catalogue download failed · showing saved catalogue\n${e.message}`:e.message}};
    function renderCatalogStatus(s){let browserLoading=s.configured&&s.revision!==libraryRevision,catalogLoading=s.refreshing||!s.configured;catalogMessage.textContent=browserLoading?(hasCachedCatalog?'Updating catalogue in browser…':'Loading catalogue into browser…'):(s.progress||'Loading Jellyfin catalogue…');reloadLibraryButton.disabled=!s.configured||s.refreshing||browserLoading;catalogProgress.hidden=!catalogLoading&&!browserLoading;if(catalogLoading&&s.totalItems>0){catalogProgress.max=s.totalItems;catalogProgress.value=Math.min(s.loadedItems,s.totalItems)}else{catalogProgress.removeAttribute('value')}let loaded=Number(s.loadedItems||0).toLocaleString(),total=Number(s.totalItems||0).toLocaleString();catalogCount.textContent=catalogLoading&&s.totalItems>0?`${loaded} of ${total} items`:s.configured&&!browserLoading?`${loaded} items`:'';if(s.error)error.textContent=s.error}
    async function catalogStatus(){if(catalogStatusBusy)return;catalogStatusBusy=true;try{let r=await fetch('/api/catalog-status'),s=await r.json();if(!r.ok)throw Error(s.error||'Unable to read catalogue status');renderCatalogStatus(s);if(s.configured&&s.revision!==libraryRevision){await refreshLibrary(s.revision);renderCatalogStatus(s)}}catch(e){error.textContent=e.message}finally{catalogStatusBusy=false}}
    async function reloadLibrary(){reloadLibraryButton.disabled=true;error.textContent='';try{await command('/api/reload-library',{});await catalogStatus()}catch(e){error.textContent=e.message;await catalogStatus()}}
    ...
    statusTabButton.onclick=()=>selectTab('status');libraryTabButton.onclick=()=>selectTab('library');backButton.onclick=()=>{details.hidden=true;libraryList.hidden=false;selectedItem=null};detailPlayButton.onclick=playSelected;pauseButton.onclick=()=>playbackCommand('/api/pause');resumeButton.onclick=()=>playbackCommand('/api/resume');stopButton.onclick=()=>playbackCommand('/api/stop');reloadLibraryButton.onclick=reloadLibrary;search.oninput=renderItems;seek.oninput=()=>{if(dragging)elapsed.textContent=clock(seek.value)};setInterval(status,1000);setInterval(catalogStatus,1000);status();restoreLibrary();catalogStatus();
    </script>
    """#
}
