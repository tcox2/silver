...
final class WebController: @unchecked Sendable {
    ...
    function renderLibrary(result){library.hidden=!result.configured;error.textContent=result.error||(!result.items.length&&result.configured?'No media found.':'');let items=result.items||[];document.querySelector('#items').innerHTML=items.map(x=>`<div class="item ${x.compatible?'':'incompatible'}"><div>${html(x.name)}<div class="muted">${html(x.detail)}</div>${x.compatible?'':`<div class="reason">Incompatible · ${html(x.incompatibilityReason)}</div>`}</div><button data-id="${html(x.id)}" ${x.compatible?'':'disabled'}>${x.compatible?'Play':'Incompatible'}</button></div>`).join('');document.querySelectorAll('[data-id]:not([disabled])').forEach(b=>b.onclick=()=>command('/api/play',{id:b.dataset.id}).catch(e=>error.textContent=e.message))}
